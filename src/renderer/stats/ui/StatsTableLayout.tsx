...
export const StatsTableLayout = ({
    ...
    <div className={`stats-table-layout grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-0 ${expanded ? 'flex-1 min-h-0 h-full' : ''} ${className}`}>
        <div className={`stats-table-layout__sidebar pr-3 ${sidebarClassName}`} style={{ borderRight: 'var(--stats-sidebar-edge-w, 1px) solid var(--border-subtle)', ...sidebarStyle }}>{sidebar}</div>
